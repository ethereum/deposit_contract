...
def serialize_value(value, typ=None):
    ...
    if isinstance(typ, str) and typ[:4] == 'uint':
        ...
        serialized_bytes = coerce_to_bytes(value) if typ == 'bytes' else b''.join([
            serialize_value(element, typ[0]) for element in value
        ])
        ...
        serialized_bytes = b''.join([
            serialize_value(getattr(value, field), subtype)
...
def hash_tree_root(value, typ=None):
    ...
    if is_basic(typ):
        ...
        return mix_in_length(
            merkleize([hash_tree_root(element, typ[0]) for element in value]),
            len(value),
        )
        ...
        return merkleize([
            hash_tree_root(getattr(value, field), subtype)
            for field, subtype in typ.fields.items()
        ])
    else:
        raise Exception("Type not recognized")
